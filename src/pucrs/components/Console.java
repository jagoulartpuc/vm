...
public class Console {
    ...
    public static void ExecutarConsoleIO() {
        ...
        if (RequestConsoleQueue.count() != 0) {
            ...
            int endereco = request.getAdress();

            if (request.getIOOperation() == RequestIOConsole.IOType.READ) {
                System.out.println("O processo" + request.getProcessID() + "está solicitando leitura de um valor que será salvo na posição de memória " + endereco);
                ...
                MemoryManager.memory[endereco] = memoryPos;
